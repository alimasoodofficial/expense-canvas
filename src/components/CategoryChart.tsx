...
import { Expense, getCategoryColor } from "@/lib/expense-data";
...
const CategoryChart = ({ expenses }: CategoryChartProps) => {
  ...
  const data = categories.map((cat) => ({
    name: cat,
    value: expenses.filter((e) => e.category === cat).reduce((sum, e) => sum + convertAmount(e.amount, e.currency_code, currency), 0),
  })).filter((d) => d.value > 0);
            ...
            {data.slice(0, 5).map((item) => (
              <div key={item.name} className="flex items-center justify-between text-sm">
                <div className="flex items-center gap-2">
                  <span className={`inline-block w-2.5 h-2.5 rounded-full`} style={{ backgroundColor: getCategoryColor(item.name) }} />
